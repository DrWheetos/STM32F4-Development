...
/**
 * @brief  Writes a signed long inside the given string, using the provided
 *         fill & width parameters.
 *
 * @param pStr   Storage string.
 * @param fill   Fill character.
 * @param width  Minimum integer width.
    ...
    signed long value)
{
  ...
  if ((absolute / 10) > 0)
  {
    if (value < 0)
    {
      num = PutSignedLong(pStr, fill, width, -(absolute / 10));
    }
    else
    {
      num = PutSignedLong(pStr, fill, width, absolute / 10);
  ...
  while (*pFormat != 0 && size < (int)length)
  {
    ...
    else
    {
      ...
      switch (*pFormat)
      {
        ...
        case 'd': num = PutSignedLong(pStr,fill, width, va_arg(ap, signed long)); break;
 ...
 *         arguments.
 *
 * @param  pFormat  Format string. i, u, d, x, X, s, c
 */
